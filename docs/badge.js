...
function githubStatusBadge_addBlock(row, svgPath, initialText, id) {
...
function githubStatusBadge_init() {
    ...
    githubStatusBadge_addBlock(row1, githubStatusBadge_githubPath, "username/reponame", "github-stats-badge--repo");
    githubStatusBadge_addBlock(row2, githubStatusBadge_tagPath, "v1.2.34", "github-stats-badge--tag");
    githubStatusBadge_addBlock(row2, githubStatusBadge_starPath, "6.9k", "github-stats-badge--stars");
    githubStatusBadge_addBlock(row2, githubStatusBadge_forkPath, "420", "github-stats-badge--forks");
}

githubStatusBadge_setupCSS();
githubStatusBadge_init();

function githubStatusBadge_setupCSS() {
...
function githubStatusBadge_updateData(userName, repoName) {
    ...
    const releaseUrl = isDevEnvironment ? 'release.json' :
    ...
    setTimeout(() => {
        document.getElementById('github-stats-badge--repo').getElementsByTagName("span")[0].innerText = userName + '/' +
            repoName;
        document.getElementById('github-stats-badge--repo').style.opacity = opacity;
    }, 50);
    ...
    fetch(repoUrl)
        ...
        .then(data => {
            if (data) {
                ...
                document.getElementById('github-stats-badge--stars').style.opacity = opacity;
                document.getElementById('github-stats-badge--forks').getElementsByTagName("span")[0].innerText =
                    data.forks.toLocaleString();
                document.getElementById('github-stats-badge--forks').style.opacity = opacity;
            }
        });
    ...
    fetch(releaseUrl)
        ...
        .then(data => {
            if (data) {
                ...
                document.getElementById('github-stats-badge--tag').style.opacity = opacity;
            }
        });
}

githubStatusBadge_updateData("swharden", "ScottPlot");